...
class CMagCmdProjectHandler(handler):
    ...
    def create_parser(self, parser: ArgumentParser):
        ...
        cmd_new_parser = subparsers.add_parser('new')
        cmd_new_parser.add_argument("path")
        cmd_new_parser.set_defaults(func=self.handle_new)
        ...
        cmd_close_parser.add_argument("--save-config", action='store_true')
    ...
    def handle_new(self, args: Namespace):

        if self.session.project != None:
            print("please close project first.")
            return

        self.session.project = CMagProjectManager.new(args.path)
    ...
    def handle_open(self, args: Namespace):
        ...
        self.session.project = CMagProjectManager.load(args.path)
    ...
    def handle_close(self, args: Namespace):
        ...
        if args.save_config:
            self.session.project.config.save()
